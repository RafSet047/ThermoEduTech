...
# https://github.com/Mingyue-Cheng/ConvTimeNet/blob/main/TSForecasting/layers/ConvTimeNet_backbone.py
...
from src.models.cnn.patch_layers import *
from src.models.cnn.revin import RevIN
...
# Cell
class ConvTimeNet(BaseModel):
    def __init__(self, 
        configs_path: str,
        state: SharedState,
        device: str = 'cpu'):
        super().__init__(configs_path, state, device)
        c_in = state.num_features
        seq_len = state.sequence_length
        context_window = state.sequence_length
        target_window = self._configs.get('target_window', 1)
        patch_len = self._configs.get('patch_len', 4)
        stride = self._configs.get('stride', 1)
        n_layers = self._configs.get('n_layers', 4)
        ...
        head_dropout = self._configs.get('head_dropout', 0.2)
        act = self._configs.get('act', 'gelu')
        norm = 'batch'
        padding_patch=None
        head_type='flatten'
        revin=False
        ...
        if head_type == 'flatten': 
            self.head = Flatten_Head(self.n_vars, d_model, patch_num, target_window, head_dropout=head_dropout)
        else:
...
class Flatten_Head(nn.Module):
    def __init__(self, n_vars, d_model, patch_num, target_window, head_dropout=0):
        super().__init__()

        self.pool = nn.AvgPool2d(kernel_size=(d_model, patch_num))
        self.flatten = nn.Flatten(start_dim=-3)
        self.linear = nn.Linear(n_vars, target_window)
...
class ConviEncoder(nn.Module):  #i means channel-independent
	def __init__(self, patch_num, patch_len, kernel_size=[11,15,21,29,39,51], n_layers=6, d_model=128, 
				 ...
				 re_param=True, re_param_kernel=3, device='cpu'):
...
class ConvEncoder(nn.Module):
	def __init__(self, kernel_size, d_model, d_ff=None, 
						...
						enable_res_param=True, n_layers=3, re_param=True, re_param_kernel = 3, device='cpu'):
		super().__init__()
...
class ConvEncoderLayer(nn.Module):
	def __init__(self, d_model:int, d_ff:int=256, kernel_size:int=9, dropout:float=0.1, 
				 activation:str="gelu", enable_res_param=True, norm='batch', re_param=True, small_ks=3, device='cpu'):
